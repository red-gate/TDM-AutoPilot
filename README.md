...
1. Get the code either by cloning the repo (prefered), or by downloding and extracting the zip file (if it is not practical to use git to clone the repo):
    ...
    ```
       git clone https://github.com/red-gate/TDM-Autopilot.git
    ```
...
- _If you do not have dbatools or rganonymize/rgsubsetter installed already, you will need to execute run-tdm-autopilot.ps1 as admin (the first time) to perform the download/install._
